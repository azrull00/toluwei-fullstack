import { Phone, MapPin, Clock, MessageCircle, Map } from "lucide-react";
...
export function ContactSection({ settings }: ContactSectionProps) {
    ...
    const mapsUrl = settings?.contactMaps;

    return (
        <section id="kontak" className="py-24 bg-[#1A1614] relative overflow-hidden">
            {/* background glows */}
            ...
            <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-14">
                    {[
                        ...
                        <AnimateOnScroll key={label} animation="fade-up" delay={delay}>
                            <div className="border border-white/8 bg-white/4 hover:bg-white/8 rounded-2xl p-6 text-center transition-colors h-full">
                                <div className="w-11 h-11 rounded-xl bg-white/8 border border-white/10 flex items-center justify-center mx-auto mb-4">
                                    <Icon className="w-5 h-5 text-[#D4AC0D]" />
                                </div>
                ...
                {/* WhatsApp CTA */}
                <AnimateOnScroll animation="scale-in" className="text-center mb-16">
                    <a
                        href={getWhatsAppUrl()}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-3 bg-[#25D366] hover:bg-[#1ebe5c] text-white font-bold text-base px-10 py-4 rounded-2xl transition-all duration-300 shadow-lg shadow-green-900/30 hover:shadow-xl hover:-translate-y-0.5"
                    >
                        <MessageCircle className="w-5 h-5" />
                        Chat via WhatsApp
                    </a>
                    <p className="text-stone-600 text-sm mt-5">Kami merespons dalam beberapa menit ⚡</p>
                </AnimateOnScroll>
